...
Base Edition은 HTTP/Bolt 프로토콜 서버, 온톨로지 관리, 스키마 진화 기능을 제공합니다.

## Downloads

[Releases](https://github.com/zeta9044/zeta4g-base/releases) 페이지에서 최신 바이너리를 다운로드하세요.

| Platform | Architecture | 파일명 패턴 |
|----------|-------------|------------|
| macOS | Apple Silicon (M1/M2/M3/M4) | `zeta4g-base-<version>-darwin-aarch64.tar.gz` |
| Linux | x86_64 (AMD/Intel) | `zeta4g-base-<version>-linux-x86_64.tar.gz` |
...
docker run -d --name zeta4g \
  ...
  -v zeta4g-data:/data \
  zeta4lab/zeta4g-base:latest start --no-auth
```
...
docker run -d --name zeta4g \
  -p 9044:9044 -p 9045:9045 \
  -v zeta4g-data:/data \
  ghcr.io/zeta9044/zeta4g-base:latest start --no-auth
```

### Docker Compose

```bash
curl -LO https://raw.githubusercontent.com/zeta9044/zeta4g-base/main/docker-compose.yml
docker compose up -d
```
...
```bash
# macOS (Apple Silicon)
curl -LO https://github.com/zeta9044/zeta4g-base/releases/latest/download/zeta4g-base-darwin-aarch64.tar.gz
tar xzf zeta4g-base-*-darwin-aarch64.tar.gz

# Linux (x86_64)
curl -LO https://github.com/zeta9044/zeta4g-base/releases/latest/download/zeta4g-base-linux-x86_64.tar.gz
tar xzf zeta4g-base-*-linux-x86_64.tar.gz
```
